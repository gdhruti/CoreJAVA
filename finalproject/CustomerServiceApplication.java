...
public class CustomerServiceApplication {
    ...
    private void handleCreateInquiry() {
        ...
        Inquiry newInquiry = new Inquiry(customerName, details);
        int result = customerServiceService.createInquiry(newInquiry);
        if (result > 0) {
    ...
    private void handleViewInquiries() {
        List<Inquiry> inquiries = customerServiceService.getInquiries();
    ...
    private void handleAddResponse() {
        ...
        int result = customerServiceService.addResponse(inquiryId, response);
        if (result > 0) {
    ...
    private void handleViewResponses() {
        ...
        int inquiryId = scanner.nextInt();
        scanner.nextLine(); // consume newline
        List<String> responses = customerServiceService.getResponses(inquiryId);
